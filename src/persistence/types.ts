 ...
 */
export type PersistenceFactory<T = unknown> = (config: T) => PersistencePlugin;
...
export interface D1Config {
  database: unknown;  // D1Database type from Cloudflare
  tableName?: string;
  namespace?: string;
}
...
export interface KVConfig {
  namespace: unknown;  // KVNamespace from Cloudflare
  prefix?: string;
...
export interface DurableConfig {
  namespace: unknown;  // DurableObjectNamespace
  objectName?: string;
}
