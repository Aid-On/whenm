...
function answerRoleQuestion(data) {
  return data.role || "I don't know";
}

function answerKnowledgeQuestion(data) {
  return data.knows ? data.knows.join(', ') : "I don't know";
}

function answerLocationQuestion(data) {
  return data.location || "I don't know";
}

function answerOwnershipQuestion(data) {
  return data.owns || "I don't know";
}

function answerQuestion(data, q) {
  if (q.includes('role')) return answerRoleQuestion(data);
  if (q.includes('know')) return answerKnowledgeQuestion(data);
  if (q.includes('where') || q.includes('\u3069\u3053')) return answerLocationQuestion(data);
  if (q.includes('own')) return answerOwnershipQuestion(data);
...
class SimpleMockProvider {
  ...
  async formatResponse(_results, question) {
...
async function test() {
  ...
  await memory.ask("What does Bob know?");

  await memory.remember("\u592A\u90CE\u304C\u6771\u4EAC\u306B\u5F15\u3063\u8D8A\u3057\u305F", "2024-01-01");
  await memory.ask("\u592A\u90CE\u306F\u3069\u3053\u306B\u4F4F\u3093\u3067\u3044\u308B\uFF1F");
}
